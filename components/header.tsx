    ...
    <>
      <header
        className={`w-full sticky top-0 z-50 transition-all duration-300 ${
          isScrolled 
            ? "h-16 bg-gradient-to-r from-[#D7281E]/90 via-[#F36C2C]/90 to-[#F7A64A]/90 backdrop-blur-lg shadow-xl" 
            : "h-20 bg-gradient-to-r from-[#D7281E] via-[#F36C2C] to-[#F7A64A]"
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-8 h-full">
          <div className="flex items-center justify-between h-full">
            <Link href="/" className="text-3xl font-bold text-white drop-shadow-sm" style={{ fontFamily: "Philosopher, serif" }}>
              Vastumaye
            </Link>
            ...
            <div className="flex items-center gap-4">
              <nav className="hidden md:flex items-center gap-2">
                {navLinks.map((link) => (
                  <Link
                    key={link.name}
                    href={link.href}
                    className={`relative text-sm font-semibold text-white hover:text-white/80 transition-colors duration-200 px-4 py-2 rounded-full
              ...
              <Link
                href="/live-sessions"
                className="hidden md:inline-flex items-center gap-2 bg-white text-[#D7281E] font-bold py-2.5 px-6 rounded-full shadow-md hover:bg-white/90 transition-all duration-300 transform hover:scale-105"
              >
              ...
              >
                <span
                  className={`block w-7 h-0.5 bg-white rounded-full transition-all duration-300 ease-in-out ${
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            animate="visible"
            exit="hidden"
            className="fixed inset-0 z-40 bg-gradient-to-br from-[#D7281E]/95 via-[#F36C2C]/95 to-[#F7A64A]/95 backdrop-blur-xl md:hidden"
          >
            <div className="h-full flex flex-col justify-between p-8 pt-24">
                  ...
                  <motion.div variants={navItemVariants} key={link.name}>
                    <Link
                      ...
                      className={`text-3xl font-bold transition-opacity ${
                        isActive(link.href) ? "text-white opacity-100" : "text-white opacity-70 hover:opacity-100"
                      }`}
                    >
                      {link.name}
                    </Link>
              ...
              >
                 <Link
                  href="/live-sessions"
                  onClick={() => setMenuOpen(false)}
                  className="w-full max-w-sm text-center bg-white text-[#D7281E] font-bold py-4 px-10 rounded-full text-lg shadow-lg"
                >
                  Book Now
                </Link>
