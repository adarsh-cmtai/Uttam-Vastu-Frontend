...
import liveSessionService, { BookingFormData } from "@/services/liveSessionService"
...
export default function LiveSessions() {
  const { t } = useLanguage();
  const packages: Package[] = t.liveSessionsPage.packages;

  const initialData: BookingFormData = { name: "", contact: "", state: "", address: "", qualifications: "", experience: "", chosenPackage: "" };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    try {
        await liveSessionService.submitBooking(formData);
        toast.success("Booking successful! Redirecting to WhatsApp...");
        
        const whatsappMessage = `
*New Live Session Booking*
--------------------------
*Name:* ${formData.name}
              ...
              <div>{submitted ? ( <div className="bg-green-500/10 border border-green-600 text-green-800 px-4 py-3 rounded mb-6 text-center">{t.liveSessionsPage.successMessage}</div> ) : (
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <button type="submit" disabled={isLoading} className="w-full bg-[#D7281E] text-white font-bold py-3 px-6 rounded-lg hover:bg-[#b82218] transition-colors duration-300 shadow-md disabled:opacity-60">
                    {isLoading ? t.liveSessionsPage.buttonSubmitting : t.liveSessionsPage.buttonSubmit}
                  </button>
                </form>
